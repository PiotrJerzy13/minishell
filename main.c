...
/*   Updated: 2024/11/19 13:41:14 by pwojnaro         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	int *last_exit_status, t_env *environment)
{
	...
	{
		fd_out = open(last_command->output_redirect,
...
{
	t_command	*current_command;
	int			requires_special_handling;

	current_command = command_list;
	requires_special_handling = 0;
	while (current_command)
	{
		if (current_command->is_pipe && current_command->next
			&& current_command->next->output_redirect)
		{
	...
	while (1)
	{
		...
		if (*input)
		{
			...
			if (command_list)
			{
				...
				{
					execute_pipeline(command_list, &last_exit_status,
						&environment);
				}
			}
		}
